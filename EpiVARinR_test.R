...
dbspf$PoidsMoyen<-rollapply(dbspf$PoidsAdj,7,sum,align="center",fill=NA)
...
# Lissage
db<-dbspf%>%
  filter(date>="2020-07-01" & date<=LastObs-OutofSample)  %>%
  na.pass()

db$index<-1:nrow(db)

db <- db %>%
  mutate(posmean = rollapply(pos,7,mean,align="center",fill=NA)) %>%
  mutate(hospmean = rollapply(hosp,7,mean,align="center",fill=NA)) %>%
  mutate(reamean = rollapply(rea,7,mean,align="center",fill=NA)) %>%
  mutate(incid_dchospmean = rollapply(incid_dchosp,7,mean,align="center",fill=NA)) 

db$cas<-db$posmean
db$cas_sm<-rollapply(db$cas,7,mean,align="center",fill=NA)/db$PoidsMoyen
...
ggplot(data=db) + 
  geom_point(aes(x=date,y=hospmean),color = "black") + 
...
# Calcul du R effectif avec le package EpiEstim
db <- db %>%
  mutate(ReBrut = pos_7j/lag(pos_7j,7)) %>%
  mutate(Re = rollapply(ReBrut,3,mean,align="center",fill=NA))
...
RestimData$date<-seq(from = as.Date(min(incid$dates)+7),
                     ...
                     by = 'day')
db<-full_join(RestimData,db)
db$REpiEstim<-db$`Mean(R)`


gRcompare<-ggplot(data=filter(db,db$date>"2020-07-01")) +
...
# Preparation du dataset pour l'estimation VAR
...
dbOutofSample$index<-1:nrow(dbOutofSample)

dbOutofSample <- dbOutofSample %>%
  mutate(posmean = rollapply(pos,7,mean,align="center",fill=NA)) %>%
  mutate(hospmean = rollapply(hosp,7,mean,align="center",fill=NA)) %>%
  mutate(reamean = rollapply(rea,7,mean,align="center",fill=NA)) %>%
  mutate(incid_dchospmean = rollapply(incid_dchosp,7,mean,align="center",fill=NA)) 

dbOutofSample$cas<-dbOutofSample$posmean
dbOutofSample$cas_sm<-rollapply(dbOutofSample$cas,7,mean,align="center",fill=NA)/dbOutofSample$PoidsMoyen
cas_sm_model<-loess(cas_sm ~ index,data = dbOutofSample ,span=0.05)
dbOutofSample$cas_sm<-predict(cas_sm_model,newdata = dbOutofSample,na.action = na.exclude)
ggplot(data=dbOutofSample) + 
  geom_point(aes(x=date,y=cas),color = "black") + 
  geom_line(aes(x=date,y=cas_sm),color = "red") 

hosp_sm_model<-loess(hospmean ~ index,data = dbOutofSample ,span=0.05)
dbOutofSample$hosp_sm<-predict(hosp_sm_model,newdata = dbOutofSample, na.action = na.exclude)
ggplot(data=dbOutofSample) + 
  geom_point(aes(x=date,y=hospmean),color = "black") + 
  geom_line(aes(x=date,y=hosp_sm),color = "red")

rea_sm_model<-loess(reamean ~ index,data = dbOutofSample ,span=0.05)
dbOutofSample$rea_sm<-predict(rea_sm_model,newdata = dbOutofSample, na.action = na.exclude)
ggplot(data=dbOutofSample) + 
  geom_point(aes(x=date,y=reamean),color = "black") + 
  geom_line(aes(x=date,y=rea_sm),color = "red")

dbOutofSample$dc <- dbOutofSample$incid_dchospmean
dc_sm_model<-loess(dc ~ index,data = dbOutofSample ,span=0.05)
dbOutofSample$dc_sm<-predict(dc_sm_model,newdata = dbOutofSample, na.action = na.exclude)
ggplot(data=dbOutofSample) + 
  geom_point(aes(x=date,y=dc),color = "black") + 
  geom_line(aes(x=date,y=dc_sm),color = "red")
...
RestimData$date<-seq(from = as.Date(min(incid$dates)+7),
                     to = as.Date(max(incid$dates)),
                     by = 'day')
dbOutofSample<-full_join(RestimData,dbOutofSample)
dbOutofSample$REpiEstim<-dbOutofSample$`Mean(R)`


gRcompare<-ggplot(data=filter(dbOutofSample,dbOutofSample$date>"2020-07-01")) +
  geom_line(aes(x=date,y=REpiEstim,color="R: EpiEstim"),size=1) +
  geom_line(aes(x=date,y=R, color="R: SPF"),size=1) +
  geom_hline(yintercept = 1, size=0.3)+
  scale_color_manual(name="",values=c("R: EpiEstim"="blue",
...
fcstR <-exp(cumsum(as.data.frame(Forecast$fcst$R)))
fcstR$date<- dateFcst
fcstRBind<-full_join(dbOutofSample,fcstR)
fcstRBind$fcst<-fcstRBind$fcst*fcstRBind$REpiEstim[fcstRBind$date==debFcst]
fcstRBind$low<-fcstRBind$lower*fcstRBind$REpiEstim[fcstRBind$date==debFcst]
fcstRBind$up<-fcstRBind$upper*fcstRBind$REpiEstim[fcstRBind$date==debFcst]
fcstRBind<-filter(fcstRBind,fcstRBind$date>=as.Date(LastObs-LengthGraph))
...
gR<-ggplot(data=fcstRBind) +
  geom_line(data=filter(db,db$date>=LastObs-LengthGraph),aes(x=date, y = REpiEstim, color = "Observé")) +
  geom_line(aes(x=date, y = REpiEstim, color = "Observé (cut-off out of sample)")) +
  geom_line(aes(x=date, y = fcst, color = "Projection")) + 
  geom_ribbon(aes(x=date, ymin = low, ymax=up, fill="Intervalle de prévision"), alpha = 0.2)  +
  geom_hline(yintercept = 1, size = 0.2) +
  scale_color_manual(name  ="", values = c("Observé" = "black", "Observé (cut-off out of sample)"="red", "Projection" = "blue")) +
  scale_fill_manual(name = "", values = c("Intervalle de prévision" = "blue" )) +
  theme_bw() + theme(plot.title = element_text(size = 11)) +
  labs(x = NULL, y = NULL , title = "Taux de reproduction effectif (Reff)")
...
fcstcas <-exp(cumsum(as.data.frame(Forecast$fcst$cas)))
fcstcas$date<-dateFcst
fcstcasBind<-full_join(dbOutofSample,fcstcas)
fcstcasBind$fcst<-fcstcasBind$fcst*fcstcasBind$cas_sm[dbOutofSample$date==debFcst]
fcstcasBind$low<-fcstcasBind$lower*fcstcasBind$cas_sm[dbOutofSample$date==debFcst]
fcstcasBind$up<-fcstcasBind$upper*fcstcasBind$cas_sm[dbOutofSample$date==debFcst]
fcstcasBind<-filter(fcstcasBind,fcstcasBind$date>=as.Date(LastObs-LengthGraph))

gcas<-ggplot(data=fcstcasBind) +
  geom_col(data=filter(db,db$date>=LastObs-LengthGraph),aes(x=date, y = cas), fill="grey",alpha = 0.4) +
  geom_line(data=filter(db,db$date>=LastObs-LengthGraph),aes(x=date, y = cas_sm, color = "Observé")) +
  geom_line(aes(x=date, y = cas_sm, color = "Observé (cut-off out of sample)")) +
  geom_line(aes(x=date, y = fcst, color = "Projection")) + 
  geom_ribbon(aes(x=date, ymin = low, ymax=up, fill="Intervalle de prévision"), alpha = 0.2)  +
  scale_color_manual(name  ="", values = c("Observé" = "black", "Observé (cut-off out of sample)"="red" , "Projection" = "blue")) +
  scale_fill_manual(name = "", values = c("Intervalle de prévision" = "blue" )) +
...
fcsthosp <-exp(cumsum(as.data.frame(Forecast$fcst$hosp)))
fcsthosp$date<-dateFcst
fcsthospBind<-full_join(dbOutofSample,fcsthosp)
fcsthospBind$fcst<-fcsthospBind$fcst*fcsthospBind$hosp_sm[dbOutofSample$date==debFcst]
fcsthospBind$low<-fcsthospBind$lower*fcsthospBind$hosp_sm[dbOutofSample$date==debFcst]
fcsthospBind$up<-fcsthospBind$upper*fcsthospBind$hosp_sm[dbOutofSample$date==debFcst]
fcsthospBind<-filter(fcsthospBind,fcsthospBind$date>=as.Date(LastObs-LengthGraph))

ghosp<-ggplot(data=fcsthospBind) +
  geom_col(data=filter(db,db$date>=LastObs-LengthGraph),aes(x=date, y = hosp), fill="grey",alpha = 0.4) +
  geom_line(data=filter(db,db$date>=LastObs-LengthGraph),aes(x=date, y = hosp_sm, color = "Observé")) +
  geom_line(aes(x=date, y = hosp_sm, color = "Observé (cut-off out of sample)")) +
  geom_line(aes(x=date, y = fcst, color = "Projection")) +
  geom_ribbon(aes(x=date, ymin = low, ymax=up), fill="blue", alpha = 0.2)  +
  scale_color_manual(name  ="", values = c("Observé" = "black","Observé (cut-off out of sample)"="red", "Projection" = "blue")) +
  scale_fill_manual(name = "", values = c("Intervalle de prévision" = "blue" )) +
...
fcstrea <-exp(cumsum(as.data.frame(Forecast$fcst$rea)))
fcstrea$date<-dateFcst
fcstreaBind<-full_join(dbOutofSample,fcstrea)
fcstreaBind$fcst<-fcstreaBind$fcst*fcstreaBind$rea_sm[dbOutofSample$date==debFcst]
fcstreaBind$low<-fcstreaBind$lower*fcstreaBind$rea_sm[dbOutofSample$date==debFcst]
fcstreaBind$up<-fcstreaBind$upper*fcstreaBind$rea_sm[dbOutofSample$date==debFcst]
fcstreaBind<-filter(fcstreaBind,fcstreaBind$date>=as.Date(LastObs-LengthGraph))

grea<-ggplot(data=fcstreaBind) +
  geom_col(data=filter(db,db$date>=LastObs-LengthGraph),aes(x=date, y = rea), fill="grey",alpha = 0.4) +
  geom_line(data=filter(db,db$date>=LastObs-LengthGraph),aes(x=date, y = rea_sm, color = "Observé")) +
  geom_line(aes(x=date, y = rea_sm, color = "Observé (cut-off out of sample)")) +
  geom_line(aes(x=date, y = fcst, color = "Projection")) +
  geom_ribbon(aes(x=date, ymin = low, ymax=up), fill="blue", alpha = 0.2)  +
  scale_color_manual(name  ="", values = c("Observé" = "black","Observé (cut-off out of sample)"="red","Projection" = "blue")) +
  scale_fill_manual(name = "", values = c("Intervalle de prévision" = "blue" )) +
...
fcstdc <-exp(cumsum(as.data.frame(Forecast$fcst$dc)))
fcstdc$date<-dateFcst
fcstdcBind<-full_join(dbOutofSample,fcstdc)
fcstdcBind$fcst<-fcstdcBind$fcst*fcstdcBind$dc_sm[dbOutofSample$date==debFcst]
fcstdcBind$low<-fcstdcBind$lower*fcstdcBind$dc_sm[dbOutofSample$date==debFcst]
fcstdcBind$up<-fcstdcBind$upper*fcstdcBind$dc_sm[dbOutofSample$date==debFcst]
fcstdcBind<-filter(fcstdcBind,fcstdcBind$date>=as.Date(LastObs-LengthGraph))

gdc<-ggplot(data=fcstdcBind) +
  geom_col(data=filter(db,db$date>=LastObs-LengthGraph),aes(x=date, y = dc), fill="grey",alpha = 0.4) +
  geom_line(data=filter(db,db$date>=LastObs-LengthGraph),aes(x=date, y = dc_sm, color = "Observé")) +
  geom_line(aes(x=date, y = dc_sm, color = "Observé (cut-off of sample)")) +
  geom_line(aes(x=date, y = fcst, color = "Projection")) +
  geom_ribbon(aes(x=date, ymin = low, ymax=up), fill="blue", alpha = 0.2)  +
  scale_color_manual(name  ="", values = c("Observé" = "black", "Observé (cut-off of sample)"="red", "Projection" = "blue")) +
  scale_fill_manual(name = "", values = c("Intervalle de prévision" = "blue" )) +
  theme_bw() + theme(plot.title = element_text(size = 11)) +
  labs(x = NULL, y = NULL, title = "Décès hospitaliers")
