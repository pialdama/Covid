---
title: "Projection EpiVAR pour le suivi l'épidémie de Covid en France"
...
$$
avec le vecteur des variables endogènes tel que : $$Y_t = \begin{pmatrix}  \Delta \log R_t \\ \Delta \log  Cas_t\\ \Delta \log Hosp_t \\ \Delta \log Rea_t \\ \Delta \log Décés  \end{pmatrix} $$ Le vecteur des résidus $u_t$ est supposé être i.i.d. tel que $\mathbb{E}[u_t] = 0$, $\mathbb{E}[u_tu_t']=\Sigma$ et $\mathbb{E}[u_tu_{t-1}']=0$. Très utilisé en économie, cette classe de modèle *statistique* est utilisée pour modéliser plusieurs séries temporelles qui sont reliées entre elles par des relations de causalité, ce qui est le cas en particulier des variables épidémiques.
...
![](gDonnees.png){width=100%}

## Projections 

On estime ensuite le modèle sur échantillon complet (*full sample*) et sur un échantillon réduit de 2 semaines (*out-of-sample*) pour produire deux projections. La première utilise toute l'information disponible à la dernière date pour laquelle on dispose d'observation pour chacune des variables tandis que la seconde n'utilise pas les deux dernières semaines d'observations pour évaluer ce qu'aurait donné une projection passée.^[Il y a cependant un caveat à cet exercice : pour se placer dans une véritable situation de projection *out-of-sample*, il faudrait réplique le travail de préparation des données décrit plus haut en tronquant la base de données. On parlerait plutôt de *pseudo out-of-sample*.]
